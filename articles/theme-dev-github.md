...
そこでGit Flowの考え方を一部採用し、以下のフローでリリースノートが完成し、そこから成果物をダウンロードできるようにしました。
...
テーマの開発は大変なことが多いです。特にレンタルサーバーによって色々なPHPのバージョンが存在するWordPressのテーマ開発はとても大変です。WordPress自体の仕様変更に翻弄されるケースも多いです。GitHub ActionsやCIの仕組みを利用すればそういったストレスを少しでも軽減できます。
この記事では簡単にストレスを軽減する3つの施策を紹介しました。
