...
あらかじめサーバーサイドで出力しておいたHTMLを`hydrate`してるので、ガクッとした動きもなく、`CLS`を意識した実装になっています。

以前は`Lit`を使ってカスタムコンポーネントをマークダウンに埋め込むアプローチを採用していたのですが、`Lit`はサーバーサイドレンダリングが少し複雑なのでなんとか`React`で実現したいと考えなんとか実現できました！！！
...
`useHydrate`を使うと、マークダウンの中で書かれたJSXを実際に動くコンポーネントに変換してくれます！
