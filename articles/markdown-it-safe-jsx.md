...
技術者であれば記事の中でもReactなりを動かして自分の技術を存分に発揮したいですよね！！

さらにあらかじめサーバーサイドで出力しておいたHTMLをフロント側で`hydrate`する仕組みになっているので、ガクッとした動きがない`CLS`を意識した実装になっています。
これもライブラリで実現したかったことの一つです！

以前は`Lit`を使ってカスタムコンポーネントをマークダウンに埋め込むアプローチを採用していたのですが、`Lit`はサーバーサイドレンダリングが少し複雑なのでフロントエンドだけで動かしていました。そのため`Lit`エレメントがready状態になるまでは記事内にその内容が表示されず記事がガクッと表示されてしまっていました。
今回はReactをサーバーサイドでレンダリングして、クライアントでhydrateすることでその問題を解消することができました。
